          ...
          <div class="col-md-8">
            ...
            <div class="card">
              ...
              <div class="card-footer">
                <pagination :data="roles" :limit="1" @pagination-change-page="getPaginationResults"></pagination>
              </div>
            </div>
            <!-- /.card -->
          </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <form @submit.prevent="editmode ? updateRole() : createRole()" @keydown="form.onKeydown($event)">
                    ...
                    <div class="form-group">
                        <!-- <select v-model="form.permissions" name="permissions[]" class="form-control" :class="{ 'is-invalid': form.errors.has('permissions') }" multiple="multiple" id="permissions">
                            <option value="" selected="" disabled="">ধরণ সিলেক্ট করুন</option>
                            <option v-for="role in permissions" v-bind:value="role.id" :selected="role.id == 1">{{ role.display_name }}</option>
                        </select>
                        <has-error :form="form" field="roles"></has-error> -->
                        <v-select placeholder="অনুমতি প্রদান" :options="permissions" label="display_name" return-object multiple v-model="form.permissions" ref='theSelect'></v-select>
                    </div>
